...
if ($isAdmin) {
    ...
    $row = $res->fetch_assoc();
    $low_stock_count = (int) ($row['total'] ?? 0);
}
?>
<nav style="background: rgba(38, 70, 83, 0.88); padding: 15px 30px; border-radius: 20px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; font-family: 'Poppins', sans-serif; box-shadow: 0 14px 30px rgba(0,0,0,0.16); border: 1px solid rgba(255,255,255,0.12); backdrop-filter: blur(10px);">
    <div style="display: flex; align-items: center; gap: 12px;">
        <span style="font-size: 24px;">Bakery</span>
        <h2 style="color: #f4a261; margin: 0; font-size: 20px;">Bakery System</h2>
    </div>

    <div style="display: flex; gap: 20px; align-items: center;">
        <a href="dashboard.php" style="color: white; text-decoration: none; font-size: 14px; transition: 0.3s;">Dashboard</a>
        ...
        <?php if ($isAdmin): ?>
            ...
            <a href="admin_dashboard.php#user-management" style="color: #f4a261; text-decoration: none; font-weight: 600; font-size: 14px;">Users</a>
